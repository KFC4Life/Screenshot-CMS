...
@section('content')
    <div class="row mb-3">
        ...
        <div class="col-md-6 mb-3">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Screenshots <span class="badge badge-dark float-right">{{ count(\App\Models\Screenshot::where('user_id', '=', Auth::id())->get()) }}</span></h4>
                    <p class="card-text">
                        A list of screenshots that are taken will show here.
                    </p>
                    @if(count(\App\Models\Screenshot::where('user_id', '=', Auth::id())->get()) > 0)
                        <p class="card-text"><small class="text-muted">Last screenshot added {{ \Carbon\Carbon::createFromTimeString(\App\Models\Screenshot::latest()->first()->created_at)->diffForHumans() }}</small></p>
                    @endif
